...
- **Stack (MCP server).** The MCP server is a route handler inside the website's
  ...
  and [mcp-auth-setup.md](content/knowledge/mcp-auth-setup.md). To run the whole
  app locally (local Postgres + faked auth loop, no external accounts) see
  [local-dev-testing.md](content/knowledge/local-dev-testing.md). Note: this Next.js
