...
- **No server-side domain state** (ADR 0003): the MCP server never persists studio/client fiscal data — that stays in the studio's files, managed by the client (Claude Code). Supabase is billing-only.
